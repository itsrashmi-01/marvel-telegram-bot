...
async def saga_pagination(client: Client, query: CallbackQuery):
    ...
    for item in saga_movies[start_idx : start_idx + items_per_page]:
        ...
        # DIRECTLY GO TO UPLOAD, SKIPPING PREVIEW
        buttons.append([InlineKeyboardButton(btn_text, callback_data=f"init_upload_{item['order']}")])
        
    nav = []
    if page > 1: nav.append(InlineKeyboardButton("⬅️ ᴘʀᴇᴠ", callback_data=f"saga_{code}_{page-1}"))
    if page < total_pages: nav.append(InlineKeyboardButton("ɴᴇxᴛ ➡️", callback_data=f"saga_{code}_{page+1}"))
...
async def init_upload(client: Client, query: CallbackQuery):
    ...
    if raw_movie:
        # Fetch TMDB metadata silently in the background
...
# ... (KEEP CHANNEL MANAGEMENT UI EXACTLY AS IT WAS) ...
